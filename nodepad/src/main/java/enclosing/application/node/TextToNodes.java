...
public class TextToNodes {
	...
	private void link(String target, String expanded, boolean expandsToChild) {
		try {
			...
			applet.observer.openFromFile(GetFacelessNodeField.getBasedirFile(target).getAbsolutePath()+"/"+NodeUtils.saferStringOf(target)+".nd");
			...
			if(expandsToChild){
				AutoExpandOneStep.oneStepForChild(expandedNodeComponent, applet.observer.getNode_components(), expanded);
			}else{
				AutoExpandOneStep.oneStepForParent(expandedNodeComponent, expanded,applet.observer.getNode_components().elements());
			}
